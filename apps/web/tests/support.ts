 ...
 * first. Activating the textarea opens the picker menu — with the free-session
 * choice composed alongside, "Add workspace" is a menu entry rather than the
 * only action, so the scenario selects it and raises the dialog. The directory
 * is staged here and adopted through the path editor, which is idempotent across
 * the repeated connects a scenario may make; creating a folder from inside the
