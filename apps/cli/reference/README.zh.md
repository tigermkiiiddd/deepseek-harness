...
`DSH_TOOLS_MODE` 为进程选择 `native`、`code` 或 `both`；其他值会导致启动失败。随附的 `standard`、`code` 与 `cordis` persona 默认使用简体中文进行面向用户的沟通，并且只在用户亲自明确要求语言时切换；技术内容、生成内容、subagent 消息与助手先前的文本都不会触发切换。`minimal` preset 把同一策略嵌入完整的软件助理提示词，并且仅组合持久 `bash` 和 `str_replace_editor`。创建 Web 会话时请选择极简模式；该 agent 不包含任何其他提示词段落或面向模型的插件，而共享的浏览器、workspace、持久化、沙箱与权限宿主保持不变。
