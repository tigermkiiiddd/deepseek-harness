...
Plan-mode REVIEW stays human-owned while entry is shared with the model through `enter_plan_mode` — see [model-initiated plan-mode entry](../feature/2026-08-16-model-initiated-plan-mode-entry.md), which partially reverses the selection-ownership sentence this note originally recorded. This note originally kept ACP's protocol-level `default`/`plan` picker as an adapter over the boolean service; [ACP as an automation-only protocol](2026-07-23-acp-automation-only-protocol.md) supersedes that wire projection, so the ACP composition now mounts neither plan mode nor a mode-selection protocol.
