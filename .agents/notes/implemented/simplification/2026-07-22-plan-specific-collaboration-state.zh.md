...
面向人类的组合拥有 plan 选择与评审，而模型通过 `enter_plan_mode` 共享进入方式——详见[模型自主的 plan mode 入口](../feature/2026-08-16-model-initiated-plan-mode-entry.zh.md)，它在某种程度上推翻了本笔记先前记录的选择归属说法。本笔记最初把 ACP 协议级的 `default`/`plan` 选择器保留为布尔服务之上的适配器；[ACP 作为仅面向自动化的协议](2026-07-23-acp-automation-only-protocol.zh.md) 取代了那个协议投影，因此 ACP 组合现在既不挂载 plan mode，也不提供模式选择协议。
