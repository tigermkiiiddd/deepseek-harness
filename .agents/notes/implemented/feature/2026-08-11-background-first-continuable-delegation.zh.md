...
- `tool:<toolName>` 系统提示词 section 会告诉模型优先委派而不是一切内联完成——宽泛探索、可并行推进的独立子任务、中间步骤会淹没对话上下文的工作都适合委派——并同时启动相互独立的委派、在它们运行时继续有用工作，且仅当下一步动作依赖结果时选择前台。当 `ask_user_question` 在同一作用域中可见时，该 section 还会告诉模型在启动体量较大的委派前先向用户确认；点名该工具是有条件的，因为指引不能指向模型无法调用的工具。只有当该工具在组装作用域中仍可见时才会渲染这个 section，因此子级工具限制会同时移除 schema 与对应指引。
