...
- a `tool:<toolName>` system-prompt section tells the model to prefer delegation over doing everything inline — broad exploration, independent subtasks that can advance in parallel, and work whose intermediate steps would flood the conversation's context — to start independent delegations together, continue useful work while they run, and choose foreground only when the next action depends on the result. While `ask_user_question` is visible in the same scope, the section also tells the model to confirm substantial delegations with the user before starting them; naming that tool is conditional because guidance must not point at a tool the model cannot call. The section renders only when that tool remains visible in the assembly scope, so a child tool restriction removes the schema and its guidance together.
