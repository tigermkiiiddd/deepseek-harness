...
- A dynamic Plugin is the wrong home for a permanent capability. When the user expects the outcome to survive restarts and the session reports a harness source checkout (the `harness:self-cognition` prompt section or the `self_cognition` tool), load the `self-development` skill and edit that checkout instead; source changes take effect on the next process start.
