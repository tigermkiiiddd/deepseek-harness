...
/** The smallest Exa-shaped answer the provider accepts — enough to observe the request. */
const ONE_RESULT = { results: [{ url: 'https://a.test', highlights: ['hi'] }] }

async function boot(config: Record<string, unknown> = { apiKey: 'exa-key', baseURL: 'https://search.entry.test' }): Promise<{ ctx: Context; settingsFiber: Fiber; pluginFiber: Fiber }> {
...
describe('web-search-exa settings section', () => {
  ...
  it('fails a search as credential-missing when no layer supplies a key', async () => {
