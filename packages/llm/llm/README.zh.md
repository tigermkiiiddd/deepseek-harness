...
询问端点属于配置期针对**草稿**的操作，以 settings namespace 而非提供方路由为键——界面正在新增的提供方还不存在，也就没有路由可点名。请求仍可**点名**它正在编辑的路由；已描述该路由的适配器默认用自己安装的 catalog 作答、无需联网——那份 catalog 是其自身模型列表的版本缓存，未必是今天的——而 `preferEndpoint: true` 则改为读取端点当前的列表并与之合并，使上游后来新增的模型也出现在候选中。路由名称和 `baseURL` 至少需要提供一项。除此之外，请求携带端点、协议，以及一条 harness 只用于这一次询问、绝不存储的凭据。这里既不读取也不写入 settings 或 credentials；返回内容是界面可以提供给用户采纳的候选元数据，而不是已注册的 catalog。`LlmDiscoveredModel` 除 `id` 外每个字段都是可选的，因为大多数提供方列表只公布 id；采纳其中一条的界面仍要补上其适配器所需的容量。发现的模型可携带 `api`（应答方为该模型使用的线路协议），当路由服务多种协议时由界面写入 profile——缺失则继承路由自身的回答。重复与不可用的 id 会被丢弃，无人服务的 namespace 以 `NO_DISCOVERY` 失败，既不点名路由也不给端点的请求以 `INVALID_DISCOVERY` 失败。
