      ...
      acme-gateway:
        ...
        models:
          ...
          - id: acme-think
            ...
            maxTokens: 32768
            # vLLM extension: cap reasoning separately while leaving the
            # visible-output maxTokens allowance unchanged.
            thinkingTokenBudget: 24576
...
### vLLM thinking-token budget

`thinkingTokenBudget` is a model-level cap for vLLM's `thinking_token_budget` OpenAI-compatible extension. It is accepted only for `openai-completions` models and is injected into every request for that model after pi-ai constructs the payload. The value is a positive integer independent of `maxTokens`: the former limits reasoning tokens, while the latter remains the visible-output allowance, so the two values may be equal. To preserve that contract on the wire, the adapter adds the reasoning budget to pi-ai's `max_completion_tokens` or `max_tokens` value, then sends the separate `thinking_token_budget`; a compaction request with both Harness values set to 8,192 therefore sends a total completion cap of 16,384. This is distinct from `thinkingBudgets`, which maps named reasoning levels for providers whose native protocol uses token budgets.
...
Resolution still fails loud, naming the offending route and model, when a route cannot be served at all: a route the catalog does not ship needs `api`, `baseURL`, and a non-empty `models` list of uniquely-identified models. That resolution runs inside the section schema, so an unserviceable profile is refused **where it is written** — `settings.mutate` answers `settings-rejected` naming the route and model — rather than being stored and then quietly disabling every route in the namespace. The settings seam keeps a namespace's last good value for an already-stored section that fails, so this cannot strand a deployment. `api` accepts the protocols in `supportedProtocols()`, at the route and again per entry. A model absent from the catalog inherits the protocol its shipped siblings agree on, so adding a model to a single-protocol catalog route restates nothing; on a route whose catalog spans several protocols — or one the catalog does not describe — an undescribed model needs an entry `api` naming the wire protocol its endpoint speaks for it, and a named entry `api` is a deliberate repoint that overrides the inherited one. The same inheritance picks the endpoint base: an undescribed model on a catalog route takes the base of a shipped sibling speaking that model's protocol, so a gateway whose anthropic models sit at `/zen/go` and its OpenAI-compatible ones at `/zen/go/v1` needs no `baseURL` for either.
...
A request naming a route the **installed catalog ships is answered from that catalog**, with no network call: pi-ai's registry is a version cache of its own providers' lists, and it carries the context windows and output caps a listing endpoint would not disclose. Such a route needs no `baseURL` at all — when one must be read over the wire, the installed entries supply it, preferring the base that ends in `/v1`. The request may still set `preferEndpoint`, which reads the endpoint's current list and joins it over the catalog: installed models keep their cached capacities and protocol in catalog order, upstream additions follow them deduplicated, and an addition to a single-protocol route inherits that protocol so adoption can write it. Only a route the catalog does not describe — a gateway, a self-hosted server — is interrogated over the wire without the flag, and one that names no endpoint is told to set one or enter its models by hand.
