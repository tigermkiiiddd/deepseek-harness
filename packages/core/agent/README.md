...
- `ctx.agents.reseed(options: ReseedAgentOptions): Promise<AgentHandle>` — rebuild the LIVE agent under the same session id from a prefix of its own log: capture the first `keepSeqs` events, dispose the live handle, truncate the durable log through session persistence, then re-create through the same publication sequence. Header fields carry over (`meta` overrides), and `agentOptions` defaults to the live agent's. Rejects when no live agent has the id or persistence is unconfigured. This is the in-place rerun seam ([rerun decision](../../../.agents/notes/implemented/bug-fix/2026-08-16-rerun-truncates-and-rebuilds-in-place.md)).
