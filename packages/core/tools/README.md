...
When `lazyLoading` is active, exact schemas for eligible tools stay server-side. The fixed prefix contains a bounded name/description catalog plus the stable `tool_search`, `tool_describe`, and `tool_call` bridges. Top-level tools, including all three bridges and `alwaysVisible` tools, are called directly and never through `tool_call`. A deferred name already present in the catalog needs only direct `tool_describe` followed by direct `tool_call`; `tool_search` is for finding a name by capability or compensating for a truncated catalog. `tool_describe` returns one exact schema in the conversation tail without changing later request prefixes. Routing a bridge or non-deferred name through `tool_call` fails with the direct-call or search correction. `listingMaxTokens` bounds the compact catalog, and `activationThresholdTokens` controls `auto`. Each preset can set this configuration through `dsh-agent-tool-presentation`; `enabled: on` enables it and `enabled: off` disables it.
...
- `ctx.tools.presentAs(mode: ToolPresentationMode, lazyLoading?: LazyLoadingConfig): () => void` selects this agent's model-facing presentation and optional progressive disclosure, shadowing deployment defaults for that agent alone; it throws from a plain context and from a second declaration in the same scope. Disposed with the calling fiber.
