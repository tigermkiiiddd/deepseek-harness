...
- `ctx.agents.reseed({ sessionId, keepSeqs, meta?, agentOptions?, setup? }): Promise<AgentHandle>` — in-place rebuild of the live agent from its log's first `keepSeqs` events: the factory disposes its own live handle, truncates the durable log, and republishes under the same id. The factory tracks every live teardown keyed by session id, so a reseed always reaches the exact handle. Header fields and `agentOptions` carry over unless overridden.
