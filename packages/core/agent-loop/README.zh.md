...
- `ctx.agents.reseed({ sessionId, keepSeqs, meta?, agentOptions?, setup? }): Promise<AgentHandle>`：以 live 日志前 `keepSeqs` 个事件原地重建：工厂 dispose 自己持有的 live handle，截断持久日志，再以同一 id 重新发布。工厂按 session id 键控跟踪每个 live teardown，reseed 总能到达确切的 handle。头部字段与 `agentOptions` 默认承继，可被覆盖。
