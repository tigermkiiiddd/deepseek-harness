...
`StructuredCompactionEngine` requires `ctx.llm`, `ctx.tokenMeter`, and `ctx.sessions`. The composition below receives `ctx.llm` from its host and installs the other two services:
...
export const name = 'compaction-structured'
export const inject = ['llm']

export function apply(ctx: Context): void {
  ctx.plugin(SessionStore)
  ctx.plugin(TokenMeter)
  ctx.plugin(StructuredCompactionEngine)
}
```
...
```yaml
- name: '@deepseek-ai/dsh-compaction-structured'
  config:
    thresholdRatio: 0.8
    retainRatio: 0.16
    modelPolicies:
...
##### Compaction facts (harness-guaranteed, injected only when present)

When the shadowed region touched at least one file, or carried a plan while plan mode was active, the harness appends the guidance below to the instruction, followed by the affected sections. They come from the logged surface, not the model, so they are authoritative:

Harness-captured facts guidance:
...
Files section (rendered only when at least one file was touched):

```markdown
## Files touched in the region (harness-guaranteed)
- config.json: read the settings
- out.txt: write the output
```

Plan section (rendered only when plan mode was active):

```markdown
## Active plan (harness-guaranteed)
Build the feature
```

> A file entry is `- <path>: <explanation>`; the explanation is the assistant text immediately before the file tool call, truncated to 200 characters and marked with `…`. A file repeated inside the region is listed once, carrying the explanation from its first touch. The active-plan section appears only when plan mode was active at the end of the region.
