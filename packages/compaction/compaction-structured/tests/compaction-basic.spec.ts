...
import StructuredCompactionEngine from '@deepseek-ai/dsh-compaction-structured'
import type { StructuredCompactionConfig } from '@deepseek-ai/dsh-compaction-structured'
import { selectCompactableRange } from '@deepseek-ai/dsh-compaction-structured/src/region.ts'
import type { SummarizationInput, SummaryResult } from '@deepseek-ai/dsh-compaction-structured/src/summarizer.ts'
import { CompactionId, toolPairingBalancedAfter, toolPairingBalancedBefore } from '@deepseek-ai/dsh-compaction'
import {
  resolveCompactSpec,
  resolveConfig,
  resolveTargetPolicy,
} from '@deepseek-ai/dsh-compaction-structured/src/config.ts'
...
class TestCompactionEngine extends StructuredCompactionEngine {
...
function service(
  config: StructuredCompactionConfig = { auto: false },
  ctx = createContext(),
...
async function compactIfNeeded(
  compact: StructuredCompactionEngine,
  session: Session,
...
describe('compact configuration and defaults', () => {
  ...
  it('validates common values and pressure-policy invariants', () => {
    const bad = [
      ...
      [{ models: { [MODEL]: { retainTokens: 10 } } }, /StructuredCompactionConfig: unknown key "models"/],
      [{ thresholdRato: 0.5 }, /StructuredCompactionConfig: unknown key "thresholdRato"/],
    ] as Array<[unknown, RegExp]>

    for (const [config, pattern] of bad) {
      expect(() => resolveConfig(config as StructuredCompactionConfig)).toThrow(pattern)
    }
...
describe('pressure measurement and retention', () => {
  const compactConfig: StructuredCompactionConfig = {
    auto: false,
    thresholdRatio: 0.5,
    retainTokens: 180,
  }
...
describe('optional model-free tool-result pruning', () => {
  ...
  it('retains the original compaction-structured behavior without the optional plugin', async () => {
    const ctx = createContext(2_000)
    const compact = new TestCompactionEngine(ctx, {
      auto: false,
      thresholdRatio: 0.5,
...
describe('region facts folded for the summarizer', () => {
  interface FileTurn {
  ...
  /**
   * Build closed turns each ending in one tool call, plus an optional closing
   * plan turn, all inside a trailing open turn so `compactRegion` can run. The
   * `plan/mode` fold is written through a structural cast because this package
   * does not load the plan-mode type augmentation while still folding it.
   * @param turns - one tool call per closed turn.
   * @param plan - when present, an active plan mode fold plus an `exit_plan_mode` turn.
   * @returns a session whose compacted region carries those facts.
   */
  function fileTurnSession(turns: FileTurn[], plan: string | undefined): Session {
    const session = Session.create(SessionId('file-turns'))
    let turn = 1
    for (const call of turns) {
      ...
      const assistantText = call.assistantText ?? `working ${turn} `.repeat(300)
      session.append('assistant/message', {
        turn,
        step: 1,
        message: createMessage({
          ...
          content: [
            ...
            { type: 'tool-call', id: callId, name: call.name, arguments: JSON.stringify(call.arguments) },
          ],
          source: {
            kind: 'model',
            ...{ provider: MODEL, model: MODEL },
      ...
      session.append('tool/call', { turn, step: 1, callId, name: call.name, arguments: JSON.stringify(call.arguments) })
      session.append('tool/result', {
        turn,
        step: 1,
        message: createToolResultMessage({
    ...
    if (plan !== undefined) {
      session.append('plan/mode' as unknown as SessionEventType, { active: true } as never)
      const callId = CallId(`call-${turn}`)
      session.append('turn/start', { turn })
      session.append('step/start', { turn, step: 1 })
      session.append('assistant/message', {
        ...
        message: createMessage({
          role: 'assistant',
          content: [
            { type: 'text', text: 'presenting plan '.repeat(300) },
            { type: 'tool-call', id: callId, name: 'exit_plan_mode', arguments: JSON.stringify({ plan }) },
          ],
          source: {
            kind: 'model',
            ...{ provider: MODEL, model: MODEL },
      ...
      session.append('tool/call', { turn, step: 1, callId, name: 'exit_plan_mode', arguments: JSON.stringify({ plan }) })
      session.append('tool/result', {
        turn,
        step: 1,
        message: createToolResultMessage({
          callId,
          content: [{ type: 'text', text: 'plan result '.repeat(300) }],
          isError: false,
        }),
      }, { surfaceOp: 'append' })
      session.append('step/end', { turn, step: 1 })
  ...
  ): Promise<SummarizationInput['facts']> {
    const compact = service()
    return compact.compactRegion(start, end, agent(session, MODEL), SIGNAL)
      .then(() => compact.calls[0]!.input.facts)
  }

  it('records each touched file in first-seen order with its adjacent explanation', async () => {
    const session = fileTurnSession([
      { name: 'read', arguments: { file_path: 'config.json' }, assistantText: 'I will read the config' },
      { name: 'write', arguments: { file_path: 'out.txt' }, assistantText: 'Now I write the result' },
    ], undefined)
    const nodes = session.surface.nodes

    const facts = await regionFacts(session, nodes[0]!, nodes[nodes.length - 1]!)
    const files = facts?.files ?? []
    expect(files.map(file => file.path)).toEqual(['config.json', 'out.txt'])
    expect(files.at(0)?.explanation).toBe('I will read the config')
    expect(files.at(1)?.explanation).toBe('Now I write the result')
    expect(facts?.plan).toBeUndefined()
  })

  it('deduplicates a path repeated across region tool calls, keeping the first explanation', async () => {
    const session = fileTurnSession([
      { name: 'read', arguments: { file_path: 'shared.txt' }, assistantText: 'first touch' },
      { name: 'open_file', arguments: { file_path: 'shared.txt' }, assistantText: 'second touch' },
    ], undefined)
    const nodes = session.surface.nodes

    const facts = await regionFacts(session, nodes[0]!, nodes[nodes.length - 1]!)
    const files = facts?.files ?? []
    expect(files.map(file => file.path)).toEqual(['shared.txt'])
  ...
  it('includes the active plan when the region presents one', async () => {
    const session = fileTurnSession(
      [
        ...
        { name: 'write', arguments: { file_path: 'out.txt' }, assistantText: 'write next' },
      ],
      '# Build the feature\n1. scaffold\n2. ship',
    )
    const nodes = session.surface.nodes
  ...
  it('omits the plan when plan mode is not active in the log', async () => {
    const session = fileTurnSession(
      [{ name: 'read', arguments: { file_path: 'config.json' }, assistantText: 'read only' }],
      undefined,
    )
    // Append a non-surface plan/mode fold that is inactive, so the region
    // still touches a file but folds to plan mode being off.
    session.append('plan/mode' as unknown as SessionEventType, { active: false } as never)
    const nodes = session.surface.nodes
  ...
  it('truncates the adjacent explanation to the harness limit, marking the cut', async () => {
    const longText = 'X'.repeat(250)
    const session = fileTurnSession([
      { name: 'read', arguments: { file_path: 'config.json' }, assistantText: longText },
    ], undefined)
    const nodes = session.surface.nodes

    const facts = await regionFacts(session, nodes[0]!, nodes[nodes.length - 1]!)
    expect(facts?.files[0]!.explanation).toBe(`${longText.slice(0, 200)}\u2026`)
  })
})
...
class ExposedCompactionEngine extends StructuredCompactionEngine {
  runSummarize(
    input: SummarizationInput,
    owner: Agent,
    signal?: AbortSignal,
...
describe('default one-shot summarizer', () => {
  ...
  it('injects harness-captured region facts as authoritative guidance before the instruction', async () => {
    const { adapter, compact } = await summarizerHarness([{ type: 'text', text: 'summary' }])
    const output = await compact.runSummarize({
      ...
      facts: {
        files: [
          { path: 'config.json', explanation: 'read it first' },
          { path: 'out.txt', explanation: '' },
        ],
        plan: '# Do the thing\n1. first step',
      },
    }, agent(conversation(1), MODEL))
    ...
    expect(text).toContain('harness-captured facts about the shadowed region')
    expect(text).toContain('Do not restate, verify, or re-derive them.')
    expect(text).toContain('Do not request tools to confirm them.')
    expect(text).toContain('Condense only the surrounding conversation narrative.')
    expect(text).toContain('## Files touched in the region (harness-guaranteed)')
    expect(text).toContain('- config.json: read it first')
    expect(text).toContain('- out.txt')
    expect(text).toContain('## Active plan (harness-guaranteed)')
  ...
  it('omits the harness-facts guidance when the region carries no facts', async () => {
    ...
    const lastText = adapter.lastOptions?.messages.at(-1)?.content[0]
    const text = lastText?.type === 'text' ? lastText.text : ''
    expect(text).not.toContain('harness-captured facts about the shadowed region')
    expect(text).not.toContain('Condense only the surrounding conversation narrative.')
...
describe('automatic listener and loader composition', () => {
  ...
  it('loads and disposes the real zero-config service stack', async () => {
    ...
    const compactFiber = await ctx.plugin(StructuredCompactionEngine, { auto: false })

    expect(ctx.get('compaction')).toBeInstanceOf(StructuredCompactionEngine)
