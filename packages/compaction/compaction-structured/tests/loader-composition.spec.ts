...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@deepseek-ai/dsh-compaction-structured', StructuredCompactionEngine],
  ])
...
describe('real Loader composition', () => {
  it('loads the shipped token-meter, pruning, and compaction-structured YAML order', async () => {
    ...
    expect(loaded.get('compaction')).toBeInstanceOf(StructuredCompactionEngine)
    expect((loaded.compaction as unknown as StructuredCompactionEngine).config).toMatchObject({
      thresholdRatio: 0.5,
      retainRatio: 0.125,
      auto: false,
    })
  ...
  it('rejects stale compaction-structured config after Schemastery normalization', async () => {
    context = new Context()
    await context.plugin(LlmRuntime)
    await context.plugin(SessionStore)
    await context.plugin(TokenMeter)
    await expect(context.plugin(StructuredCompactionEngine, {
      models: { legacy: { thresholdRatio: 0.5 } },
    } as never)).rejects.toThrow(/StructuredCompactionConfig: unknown key "models"/)
  })
