 ...
 *
 * @module @deepseek-ai/dsh-compaction-structured/summarizer
 */
...
/**
 * Appended to {@link COMPACTION_INSTRUCTION} when the region carries facts,
 * telling the model the injected file list and plan are harness-guaranteed so
 * it condenses only the surrounding narrative rather than restating them.
 */
const COMPACTION_FACTS_GUIDANCE = [
...
/**
 * Render harness-captured region facts as the model-facing text they are
 * injected under: a file list when any were touched, and the active plan when
 * present, each under a headed, harness-guaranteed section.
 * @param facts - harness-captured region facts.
 * @returns the ordered sections joined for inclusion in the compaction instruction.
 */
function formatRegionFacts(facts: RegionFacts): string {
  const sections: string[] = []
  if (facts.files.length > 0) {
    const list = facts.files
      .map(file => `- ${file.path}${file.explanation.length === 0 ? '' : `: ${file.explanation}`}`)
      .join('\n')
    sections.push(`## Files touched in the region (harness-guaranteed)\n${list}`)
  }
...
/**
 * Harness-captured facts about the shadowed region, computed independently of
 * the model and injected verbatim: the files the region touched and, when plan
 * mode is active, the latest presented plan. The model treats them as sources
 * of truth and condenses only the surrounding narrative.
 */
export interface RegionFacts {
  /** Files touched in the region, in first-seen order, each with an adjacent explanation. */
...
export interface SummarizationInput {
  ...
  /** Harness-captured region facts the summarizer injects verbatim and reattaches. */
...
export async function summarizeWithLlm(
  ...
  if (target === undefined) {
    throw new Error(
      'no provider/model available for summarization: set both StructuredCompactionConfig summarization fields, route one request, or set both AgentOptions fields',
    )
  }

  const assembler = new BlockAssembler()
  const factsBlock = input.facts === undefined ? undefined : formatRegionFacts(input.facts)
  const compactionInstruction =
    factsBlock === undefined
      ? COMPACTION_INSTRUCTION
      : `${COMPACTION_INSTRUCTION}\n\n${COMPACTION_FACTS_GUIDANCE}\n\n${factsBlock}`
  const messages: Message[] = [
    ...input.messages,
    createUserMessage({
      content: [{ type: 'text', text: compactionInstruction }],
      source: { kind: 'plugin', plugin: 'dsh-compaction-structured' },
    }),
  ]
