/**
 * Load-time validation and routed-model policy resolution for compaction-structured.
 *
 * @module @deepseek-ai/dsh-compaction-structured/config
 */
 ...
 */
export function resolveConfig(config: StructuredCompactionConfig = {}): ResolvedConfig {
  validateKeys(config, BASIC_COMPACT_CONFIG_KEYS, 'StructuredCompactionConfig')
  validatePolicy(config, 'StructuredCompactionConfig')
  if (config.auto !== undefined && typeof config.auto !== 'boolean') {
    throw new Error('StructuredCompactionConfig: auto must be a boolean')
  }

  const thresholdRatio = config.thresholdRatio ?? DEFAULT_THRESHOLD_RATIO
  const retention = resolveRetention(config, { retainRatio: DEFAULT_RETAIN_RATIO })
  validateRatioRetention(thresholdRatio, retention, 'StructuredCompactionConfig')
  ...
  for (const [index, policy] of modelPolicies.entries()) {
    validateRatioRetention(
      ...
      `StructuredCompactionConfig: modelPolicies[${index}]`,
    )
  }
...
export function resolveCompactSpec(
  ...
  if (!Number.isInteger(contextWindow) || contextWindow <= 0) {
    throw new TargetPressureConfigError(
      targetKey,
      `StructuredCompactionConfig: contextWindow (${contextWindow}) must be a positive integer`,
    )
  }
  ...
  if (retainTokens >= thresholdTokens) {
    throw new TargetPressureConfigError(
      targetKey,
      `StructuredCompactionConfig: ${policy.target.provider}/${policy.target.model} retainTokens `
...
function resolveModelPolicies(configured: unknown): ModelCompactPolicyConfig[] {
  ...
  if (!Array.isArray(configured)) {
    throw new Error('StructuredCompactionConfig: modelPolicies must be an array')
  }
  const seen = new Set<string>()
  return configured.map((source: unknown, index) => {
    const name = `StructuredCompactionConfig: modelPolicies[${index}]`
    assertModelPolicy(source, name)
    const key = `${source.provider}\u0000${source.model}`
    if (seen.has(key)) {
      throw new Error(
        `StructuredCompactionConfig: duplicate model policy for ${source.provider}/${source.model}`,
      )
    }
    seen.add(key)
    return { ...source }
