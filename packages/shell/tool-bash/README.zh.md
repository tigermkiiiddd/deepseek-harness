...
| `workdir` | string | 本次调用的工作目录。默认为调用方会话有效 cwd 的文件系统标识（固定会话用不可变 `header.cwd`，自由会话用 `set_cwd` 的值），使每个会话都在自己的工作区中运行；相对 `workdir` 也以同一标识为基准解析。还没有目录的自由会话会以 `no session working directory: pass an absolute workdir, or set one with the set_cwd tool` 拒绝调用，而不是回退到 `process.cwd()`。 |
...
执行前，`command`、`workdir` 和 `timeoutMs` 会通过 `ctx.shell.resolve()` 依据执行器配置默认值完成解析，因此 Service Definition（`ShellExecSpec`）收到显式的 `workdir`/`timeoutMs` 值。工具层会根据调用方会话的有效 cwd（`currentSessionCwd`，来自 `dsh-session`）应用工作目录默认值，然后才调用 `resolve()`：由于 N 个会话共享一个执行器，逐会话 cwd 必须来自 `exec.agent`；只有真正无 agent 的调用才回退到执行器自身配置／`process.cwd()`。还没有目录的自由会话会被拒绝，而不是静默从 `process.cwd()` 运行。存在沙箱策略时，工具会复用已经规范化的 `workspaceRoot` 作为工作目录基准，防止限制逻辑与进程启动过程对同一个会话路径拼写产生不同解析结果。
