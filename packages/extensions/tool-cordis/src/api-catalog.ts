...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: 'async readEntries(id: string): Promise<readonly { id: string; name: string; disabled: boolean }[]>',
        description: 'Read one preset\'s plugin entry list as the browser sees it.\n\nGroup rows are flattened so the returned list lines up one-to-one with the plugins a session running this preset would load; a child\'s effective disabled state folds in any ancestor group that is disabled. `!!js` expressions in `disabled` are evaluated against the global scope so the browser sees the same effective on/off state the loader would at mount time (the loader evaluates `!!js` in the entry\'s fiber scope, which falls through to globals for identifiers like `process`).',
        parameters: [{ name: 'id', description: 'the preset id.' }],
