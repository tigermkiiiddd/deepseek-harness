...
- `member_add` — spawn a member process at runtime, persist it in the durable roster, and join it. Accepts the full member config: `kind`, `command`, `args`, `cwd`, `env` (layered over the full parent environment), `permission` (`allow` / `reject` fallback policy), `autostart`, and — for `kind: "dsh"` members — `preset`: a unique agent preset composition written into the member's private home at creation, which becomes its default.
