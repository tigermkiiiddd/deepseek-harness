...
export function SessionNodeItem({ node, currentId, now, onOpen, onRename, onFork, onArchive, drag, flat = false, t }: {
  ...
  const [menuOpen, setMenuOpen] = useState(false)
  // Rename and fork route through the session backend (member topics fork via
  // native session/fork and rename via the dsh extension), so every session
  // row offers the same verbs. Archive hides the row through the
