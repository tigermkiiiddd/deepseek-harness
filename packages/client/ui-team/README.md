...
- **Browser half** (`src/client/index.ts`): the status push bridge subscribes to forwarded `team/status` remote events and folds them into one `TeamController` store (exposed through the inject `hooks` compartment as `useTeamLive`). The global lane (`TeamTopbar`, an SVG node graph) reads that store and drives `api.team.*` through the formal host API (`@deepseek-ai/dsh-client-connection`). Clicking a member node resolves the member's latest topic (`team.sessions`) and selects the `member:<memberId>:<topicId>` session through `ctx.sessions.open`; if the member has no topics, the controller creates one (`team.newSession`) and then selects it. Clicking the main-instance node returns to the main conversation view (`ctx.sessions.clear`). Nothing polls.
