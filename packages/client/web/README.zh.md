...
- **有意采用一次性渲染**：UI 等待启动 settle；只要一个配置项失败，加载页面就会保留并逐项显示醒目的报告，不提供部分可用性（渐进式渲染将作为独立项目恢复）。settle 之后的渲染失败（某个服务被撤走、拆掉了 UI 脚下的组装——断连、热重载）经 AppRoot 的崩溃边界落到同一张醒目报告卡上；此后每次 fiber 状态变化都会重试真实 UI，服务图恢复后无需刷新页面即可还原界面。
