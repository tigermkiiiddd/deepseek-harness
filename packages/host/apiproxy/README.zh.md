...
会话标题与其他所有领域一样搭乘这对通用投影机制——历史尾页的 `projections` 块外加 `title` 键下的 `session/projection` 帧。本地会话的标题不会加入 `session.list`；冷会话在其中仍只有元数据，直到打开或恢复操作附加其日志。成员会话是例外：每个 `member:<memberId>:<topicId>` 行都携带一个合成 `title` 投影，由成员的显示标题与话题自身的标题或短话题 id 组成（`<memberTitle> · <topic title or id>`），`asOfSeq` 为 `-1`，让新连接的列表直接把标题种进 client 的标题格。`session.rename` 接受用户显式标题（冷会话先恢复），委托给 `ctx.sessionTitle.rename`——被接受的 `session/title` 事件将标题钉住、不再被自动生成覆盖——并返回规范化后的标题及其事件 seq，让 client 在推送帧到达前就结算自己的 `title` 投影格；规范化后为空的标题返回 `title-invalid`。
...
团队成员会话把 `team` 服务的 ACP 话题桥接进主会话 UI。组合了 `@deepseek-ai/dsh-team` 时，每个成员话题以虚拟 session id `member:<memberId>:<topicId>` 暴露：它携带合成的 `title` 投影出现在 `session.list` 中，接受 `session.prompt` 与 `session.cancel`，经 ACP 反向翻译器提供 `session.history`，并在 mux 流上发出 `session/event`、`session/projection` 与 `approval/requested` 帧。成员离线时，列表与历史会回退到该成员的持久缓存；未知话题仍以 `session-not-found` 失败。会改动成员自身话题的操作——`session.fork`、`session.rerun`、`session.rename`、`session.attachment` 与会话日志导出——一律以不支持拒绝，因为成员拥有自己的会话，harness 只是投影它们。
