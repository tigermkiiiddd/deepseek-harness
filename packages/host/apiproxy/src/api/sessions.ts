...
export interface SessionsApi {
   ...
   * `cwd` is accepted; an omitted project uses the Host cwd. `cwd: null` is
   * the explicit free-session request: the header records no working
   * directory and the session's effective directory is chosen at runtime
   * through the `set_cwd` tool. A caller may
   ...
   */
  create(request: RpcRequest<{ workspaceId?: WorkspaceId; cwd?: string | null; sessionId?: SessionId; agentPreset?: string }>):
