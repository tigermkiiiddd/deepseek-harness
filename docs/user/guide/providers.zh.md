...
在**模型目录**中选择**刷新模型**，会查询表单当前显示的基础 URL 和凭据：自定义提供方的模型列表被端点返回的清单直接替换（草稿状态，保存前不会存储提供方）；内置目录提供方则从它自己的端点重新同步并写入本地缓存，路由保持内置身份，不会被标记为自定义。刷新成功在页面显示「已同步 N 个模型」；端点不可达时错误显示在模型行旁边，列表仍可手工编辑。
...
- **刷新模型返回 401**：检查密钥。模型发现会调用 OpenAI 兼容的 `GET /models` 端点；对于不提供该端点的服务，请手动输入模型。
